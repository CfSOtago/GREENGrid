...
t <- ftfSafeDT[, .(obsStart = min(rDateTime, na.rm = TRUE), # allows for broken GPS Date/Time
                   ...
                   nObservations = .N),
               keyby = .(evID)]
kableExtra::kable(caption = "Start and end of observation period and number of obs by anonymised vehicle ID", t) %>% kable_styling()
```
...
kableExtra::kable(t, caption = "Check coding") %>% 
  kable_styling()
