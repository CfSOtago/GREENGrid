...
kableExtra::kable(caption = "Number of obs and mean pack amps where time cannot be set by original GPS Date and Time", t) %>% kable_styling()
...
Finally (Figure \@ref(fig:checkPower)) shows the distirbution of the derived -ve, zero and +ve power values using the following filters:
...
```{r powerTable}
ftfSafeDT <- ftfSafeDT[, chargingFlag := "Not charging (in use)?"]
...
> XX looks like rTime is wrong tz? Regen should be during the day? XX

```{r plotChargingTime, fig.cap="Inferred charging times and power draw (all data)"}
plotDT <- ftfSafeDT[, .(meanPowerW = mean(powerW)), keyby = .(rTime, rDow,derivedLocation, evID, chargingFlag)]
