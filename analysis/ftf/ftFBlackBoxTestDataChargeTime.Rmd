...
Clearly we need to do some outlier analysis and possibly some data cleaning. We also need to seperate regeneration properly by checking for speed = 0. 
...
We assume that -ve amps (and so -ve power) is current flowing in and we filter out observations where speed > 0 (see above). We use `Speed (Speedometer)` as this has no missing values unlike `Speed (GPS)`.

```{r powerTable}
ftfSafeDT <- ftfSafeDT[, chargingFlag := "Not charging"]
...
kableExtra::kable(dt, caption = "Summary of derived chargeingPowerW per car") %>% 
  kable_styling()
```

Visualise as a simple daily trend line for Grid charging only.

```{r powerTrends}
plotDT <- ftfSafeDT[chargingFlag == "Grid", .(meankW = mean(powerW)/1000), keyby = .(evID, rDate)]
...
plotDT <- ftfSafeDT[chargingFlag == "Grid", .(meanPowerW = mean(powerW)), keyby = .(rTime, rDow,derivedLocation, evID)]
...
What does Figure \@ref(fig:plotChargingTime) show?

Where is the non-home charging happening? We need a way to infer other locations without being disclosive...
