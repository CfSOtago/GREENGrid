 ...
 * [Report history](https://git.soton.ac.uk/ba1e12/nzGREENGrid/commits/master/analysis/ggDemandProfiles/nzGGProfileAnalyserTemplate.Rmd)
 ...
 * `r params$circuitPattern` circuit data pre or newly extracted from the cleaned safe data using https://github.com/dataknut/nzGREENGridDataR/tree/master/dataProcessing/gridSpy/extractCleanGridSpy1minCircuits.R
...
knitr::kable(caption = "Example household attribute data for sample", head(hhAttributesDT))
```
...
knitr::kable(caption = paste0("Counts of ", circuitPattern, " observations by label"), t)
```
...
knitr::kable(caption = paste0("Counts of ", circuitPattern, " labels by household"), t)
```
...
```{r testReuse}
t <- hhAttributesDT[!is.na(notes), .(notes = unique(notes)), keyby = .(hhID, newID, r_stopDate)]

knitr::kable(caption = paste0("Counts of ", circuitPattern, " labels by household"), t)
```

In order to correctly link household attribute and grid spy data we therefore need to set newID in the `r circuitPattern` data for the appropriate dates.
...
knitr::kable(caption = paste0("Summary observation stats by hhID (sorted by date last heard from) for: ", circuitPattern), dt)
```
...
# add error bars
labelProfilePlot(ggplot(gs1MinDT, aes(x = obsHourMin, colour = season, y = powerW)) + stat_summary(fun.data = "mean_cl_boot") +
                   geom_point() +
                   labs( caption = paste0(plotCaption,"\nn households = ", nHHs,
                                                 "\nError bars = 95% CI"))
                 )
...
Figure \@ref{fig:meanPlot} shows the mean power for circuits containing "`r circuitPattern`" by season and presenceo f children. This uses the newID to correctly link all household attribute data to the grid spy data.
