...
t <- testDT[, .(Mean_W = mean(meanW),
                           ...
                           n_HHs = uniqueN(linkID)), keyby = .(group)]


sumW <- t[group == "S", Mean_W]
winW <- t[group == "W", Mean_W]
...
yp001Ref <- plotDT[pValue == "p = 0.01" & sampleN == 1000] # for reference line
x001 <- mean(yp001Ref$sampleN)
yp005Ref <- plotDT[pValue == "p = 0.05" & 
