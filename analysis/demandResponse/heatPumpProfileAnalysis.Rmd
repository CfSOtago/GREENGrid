...
myPlot <- ggplot2::ggplot(Mergedsc0DT, aes(x = obsHalfHour)) +
  ...
  theme(text = element_text(family = "Cambria")) +
  ggtitle("Costs baseline scenario") +
  facet_grid(season ~ .) +
...
myPlot <- ggplot2::ggplot(Mergedsc3DT, aes(x = obsHalfHour)) +
  geom_point(aes(y=ecoValueHH, color= Region), size=1.5) +
  theme(text = element_text(family = "Cambria")) +
  ggtitle("Economic value of load shifting by region") +
  facet_grid(season ~ .) +
  labs(x='Time of Day', y='Economic value $/MWh') +
  scale_x_time(breaks = c(hms::as.hms("00:00:00"), hms::as.hms("03:00:00"), hms::as.hms("06:00:00"),
                          hms::as.hms("09:00:00"), hms::as.hms("12:00:00"),
...
```
```{r r economic visualisation SC2 2}
...
```{r economic visualisation SC3 2}


#Plots benefits load shifting by acknowledging difference between baseline and icreased amount -> reduces savings therefore
#EcoVaLs displays the difference between increased consumption and baseline already 

EcoVaSumDT <- copy(EcoVaHHDT)

Test1 <- EcoVaSumDT[, c("season", "Region", "Period", "EcoVaLs")]
Test1 <- Test1[, EcoOP := ifelse(Period == "Off Peak 1" | Period == "Off Peak 2",
                                 EcoVaLs, 0)]
Test1 <- Test1[, EcoPeak := ifelse(Period == "Morning Peak" |
                                     Period == "Evening Peak", EcoVaLs, 0)]
Test1 <- Test1[, EcoOP := sum(EcoOP), keyby = .(Region, season, Period)]
Test1 <- Test1[, EcoPeak := sum(EcoPeak), keyby = .(Region, season, Period)]
Test1 <- Test1[, Result := ifelse(Period == "Morning Peak",
                                  "Morning Peak" - "Off Peak 2", 0)]

EcoVaSumSC3DT <- EcoVaSumDT[, Test := ifelse(Period == "Morning Peak", rowsum(EcoVaLs, "Off-Peak 1", reorder = FALSE), 


Maybe with creating variables???
