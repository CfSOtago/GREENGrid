...
#Creating new microsimulation DT

MicroSimWinterHWDT <- HotwaterObsDT
MicroSimWinterHWDT <- MicroSimWinterHWDT[, dateTime := ymd_hms(r_dateTime)]

MicroSimWinterHWDT <- MicroSimWinterHWDT[!(dateTime<="2015-07-26 11:59:00")]


MicroSimWinterHWDT <- MicroSimWinterHWDT[!(MicroSimWinterHWDT$dateTime < "2015-07-26 11:59:00"),]
...
MicroSimWinterHWDT <- subset(MicroSimWinterHWDT, `dateTime` >= "2015-07-26 12:00:00" & `dateTime` <= "2015-08-02 00:00:00")
...
MicroSimSummerHWDT <- HotwaterObsDT
MicroSimSummerHWDT <- subset(MicroSimSummerHWDT, `r_dateTime` >= "2016-2-15 00:00:00" & `r_dateTime` <= "2016-02-21 23:59:00")#Make sure there are no public holidays or school holidays


#Next steps: Defining peak and off peak, conduct load shifting, plot profiles
...
MicroSimSummerHWDT <- MicroSimSummerHWDT[obsHourMin >= OP2S & 
                     obsHourMin <= OP2E,
                   Period := "Off Peak 2"]

MicroSimSummerHWDT <- MicroSimSummerHWDT[, Period := "Not Peak"]

MicroSimSummerHWDT <- MicroSimSummerHWDT[obsHourMin >= MPS & 
                     obsHourMin <= MPE,
                   Period := "Morning Peak"]

MicroSimSummerHWDT <- MicroSimSummerHWDT[obsHourMin >= EPS & 
                     obsHourMin <= EPE,
                   Period := "Evening Peak"]

MicroSimSummerHWDT <- MicroSimSummerHWDT[obsHourMin >= OP1S & 
                     obsHourMin <= OP1E,
                   Period := "Off Peak 1 (Morning)"]

MicroSimSummerHWDT <- MicroSimSummerHWDT[obsHourMin >= OP12S & 
                     obsHourMin <= OP12E,
                   Period := "Off Peak 1 (Evening)"]
...
legend_title <- "Period"
myPlot <- ggplot2::ggplot(subset(MicroSimWinterHWDT, hhID %in% c("rf_06"))
                          , aes(x = obsHourMin)) +
  geom_line(aes(y=powerW, colour=Period), size=0.6) +
  theme(text = element_text(family = "Cambria")) +
  ggtitle("Hot water power ") +
  scale_color_manual(legend_title, labels = c("Off Peak 1 (Morning)",
                                              "Morning Peak",
                                              "Off Peak 2",
                                              "Evening Peak",
                                              "Off Peak 1 (Evening)"), values = 
                                                                c("#332288",
                                                                  "#88CCEE",
                                                                  "#44AA99",
                                                                  "#117733",
