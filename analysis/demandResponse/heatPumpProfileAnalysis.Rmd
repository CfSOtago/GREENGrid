...
CpdSumDT <- CpdMinAll[, .(SumMins = sum(CPDmin), 
                          totalMins = .N * 30), 
                      keyby = .(season, obsHalfHour)]
...
HPprofileDT <- heatPumpProfileDT[ !(season %in% c("Spring", "Summer"))]
HPprofileDT <- HPprofileDT[, perHhkWh := ((scaledGWh)*1000*1000)/nzHHheatPumps]
HPprofileHHDT <- HPprofileDT[, .(sumHhkWh = sum(perHhkWh)), keyby = .(season, obsHalfHour)]
...
CpdMergedHPDT <- CpdMergedHPDT[, totalSumMins := sum(SumMins)]

CpdMergedHPDT <- CpdMergedHPDT[, sumMeanConKwh := sum(sumHhkWh * Probability)]
CpdMergedHPDT <- CpdMergedHPDT[, CpdkW := sumMeanConKwh / (totalSumMins/60)]


0.06465411*PS1*365

#Hot water double check

HWprofileDT <- hotWaterProfileDT[ !(season %in% c("Spring", "Summer"))]
HWprofileDT <- HWprofileDT[, perHhkWh := ((scaledGWh)*1000*1000)/nzHHhotWater]
HWprofileHHDT <- HWprofileDT[, .(sumHhkWh = sum(perHhkWh)), keyby = .(season, obsHalfHour)]
...
CpdMergedHWDT <- CpdMergedHWDT[, totalSumMins := sum(SumMins)]

CpdMergedHWDT <- CpdMergedHWDT[, sumMeanConKwh := sum(sumHhkWh * Probability)]
CpdMergedHWDT <- CpdMergedHWDT[, CpdkW := sumMeanConKwh / (totalSumMins/60)]


0.08144702*PS1*365

(0.08144702 + 0.0646541) *PS1*365
