...
MicroSimDT <- MicroSimDT[, .(meankWh = mean(powernew)), keyby = .(hhID, obsHalfHour)]#first average
MicroSimDT <- MicroSimDT[, .(meankWh = mean(meankWh)), keyby = .(obsHalfHour)]#second average
MicroSimDT <- MicroSimDT[, meankWh := meankWh/2/1000]#Power W to kwh/hh
MicroSimDT <- MicroSimDT[, meankWh := meankWh * 13]#Creating seasonal average 52 weeks in the year, 13 weeks per season
...
MiShifCombHWDT <- rbind(MiSimSpringHWDT, MiSimSummerHWDT, MiSimAutumnHWDT, MiSimWinterHWDT, MicroOrigPowerDT)
...
legend_title <- "Period"
myPlot <- ggplot2::ggplot(MiCombHWDT,
                          aes(x = obsHalfHour, colour=Period, group = Period)) +
  geom_point(aes(y=meankWh), size=0.6) +
  geom_path(aes(y=meankWh))+
  geom_line(aes(y=meankWh), size=0.6)+
  theme(text = element_text(family = "Cambria")) +
  scale_color_discrete(breaks=c("Off Peak 1 (Morning)",
                                "Morning Peak",
                                "Off Peak 2",
