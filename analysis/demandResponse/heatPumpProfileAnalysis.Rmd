...
LightObsDT <- LightObsDT[, dateTime := lubridate::dmy_hm(r_dateTime, tz = "Pacific/Auckland")]
LightObsDT <- LightObsDT[, year := lubridate::year(dateTime)]
LightObsDT <- LightObsDT[, obsHalfHour := hms::as.hms(dateTime)]
LightObsDT <- LightObsDT[, month := lubridate::month(dateTime)]
LightObsDT <- LightObsDT[month >= 2 | month == 4, month == 5, season := "Autumn"]
LightObsDT <- LightObsDT[month >= 3 | month == 4, month == 5, season := "Autumn"]
LightObsDT <- LightObsDT[month == 6 | month == 7 | month == 8, season := "Winter"]

#LightObsDT <- GREENGridData::addNZSeason(LightObsDT)

LightDT <- LightObsDT[, .(meanW = powerW),
                           keyby = .(season, obsHourMin = hms::hms(r_dateTime))]
```
