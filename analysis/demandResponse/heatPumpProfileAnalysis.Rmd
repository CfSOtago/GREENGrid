...
if(userName == "carsten.dortans"){
  ggParams$dataLoc <- path.expand("/Volumes/hum-csafe/Research Projects/GREEN Grid/cleanData/safe/gridSpy/1min/profiles/") # <- server
}
```
...
LightingHCS  <- readr::read_csv("/Volumes/hum-csafe/Research Projects/GREEN Grid/cleanData/safe/gridSpy/1min/dataExtracts/Lighting_2015-04-01_2016-03-31_observations.csv.gz")
...
consumOrigPD <- LightDT[, sum(meanW/60)/1000] #Consumption in kWh for an avg day per season for one household
consumOrigTotPD <- LightDT[, sum((meanW/60)/1000/1000/1000) * nzHouseholds] #Consumption in kWh for an avg day per season for total NZ
consumOrigPY <- LightDT[, sum(meanW/60*90)/1000] #Consumption in kWh per season for one household
consumOrigTotPY <- LightDT[, sum((meanW/60*90)/1000/1000/1000) * nzHouseholds] #Consumption in kWh per season for total NZ
consumLEDPD <- LightDT[, sum(meanW*LEDfac)/60/1000]#Consumption in kWh for an avg day per season for one household LED 100%
consumLEDTotPD <- LightDT[, sum((meanW*LEDfac)/60/1000/1000/1000) * nzHouseholds]#Consumption in kWh for an avg day per season for total NZ LED 100%
consumLEDPY <- LightDT[, sum(meanW*LEDfac*90)/60/1000]#Consumption in kWh per season for one household LED 100%
consumLEDTotPY <- LightDT[, sum((meanW*LEDfac*90)/60/1000/1000/1000) * nzHouseholds]#Consumption in GWh per season for total NZ LED 100%

```

##Visualisation
```{r Insert variables from above}
...
myPlot <- ggplot2::ggplot(LightDT, aes(x = obsHourMin)) +
  geom_line(aes(y=meanW, color="red"), size=0.5) +
  geom_line(aes(y=meanWLed, color="blue"), size=0.5) +
  theme(text = element_text(family = "Cambria")) +
  ggtitle("Lighting power for an average day per season and household") +
  scale_colour_manual(name = element_blank(), 
                      values = c('red'='red', 'blue'='blue'),  labels = c('LED technology', 'Current technologies')) +
  facet_grid(season ~ .) +
  labs(x='Time of Day', y='Power (W)') +
  #scale_y_continuous( limits = c(0,3.5), expand = c(0,0) )+
  #scale_y_continuous(breaks = c(0.1)) +
...
#ggsave("Lighting power for an average day per season and household.jpg", dpi=600)
