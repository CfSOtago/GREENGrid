...
LightingHCS  <- readr::read_csv("/Volumes/hum-csafe/Research Projects/GREEN Grid/cleanData/safe/gridSpy/1min/dataExtracts/Heat Pump_2015-04-01_2016-03-31_observations.csv.gz")
LightObsDT <- data.table::as.data.table(LightingHCS)
lubridate::tz(LightObsDT[1,r_dateTime])
LightObsDT <- LightObsDT[, nzTime := lubridate::with_tz(r_dateTime, tzone = "Pacific/Auckland")]
LightObsDT <- LightObsDT[, obsTime := hms::as.hms(r_dateTime)]
LightObsDT <- LightObsDT[, obsTimeNZ := hms::as.hms(nzTime)]
LightObsDT <- LightObsDT[, year := lubridate::year(r_dateTime)]
LightObsDT <- LightObsDT[, obsHalfHour := hms::as.hms(r_dateTime)]
...
#Building seasonal daily average

LightDT <- LightObsDT[, . (meanW = mean(powerW), keyby =. (obsTime, month))]
...
#LightDT <- LightObsDT[, .(meanW = powerW),
                      #     keyby = .(season, obsTimeNZ = hms::hms(r_dateTime))]
```
...
LightDT <- LightDT[, orikWh := meanW/60/1000]
LightDT <- LightDT[, LedkWh := meanWLed/60/1000]
...
##Visualisation
```{r Original wattage and LED wattage 2in1 per household}
...
myPlot <- ggplot2::ggplot(LightDT, aes(x = obsHourMin)) +
  ...
  theme(text = element_text(family = "Cambria")) +
  ggtitle("Lighting power for an avergae day per season and household") +
  scale_colour_manual(name = element_blank(), 
