...
HPprofileDT <- heatPumpProfileDT[ !(season %in% c("Spring", "Summer"))]
HPprofileDT <- HPprofileDT[, perHhkW := (((scaledGWh*60)*1000*1000)/nzHHheatPumps)/90] ###########WARNING per day
...
CpdMergedHPDT <- CpdMergedHPDT[, Result := sum(MeanConKw), keyby = .(season)]
...
HWprofileDT <- HWprofileDT[, .(sumGWhperHalfHour = sum(scaledGWh)),
                           keyby = .(season, obsHalfHour)]
HWprofileDT <- HWprofileDT[, kWperHh := (((sumGWhperHalfHour *
                                                2)*1000*1000)/nzHHhotWater)/90]#kW per household and half hour
...
#Calculating mean half hour kW at congestion periods
CpdMergedHWDT <- CpdMergedHWDT[, ConKw := kWperHh * Probability]
#CpdMergedHWDT <- CpdMergedHWDT[, Result := sum(MeanConKw), keyby = .(season)]
...
#Claculating annual charges heat pump for one household
CpdMergedHWDT <- CpdMergedHWDT[, CPDdemAut := ifelse(season == "Autumn", ConKw, 0)]
CpdMergedHWDT <- CpdMergedHWDT[, CPDdemWin := ifelse(season == "Winter", ConKw, 0)]

sumCPDdemHWAut <- (sum(CpdMergedHWDT$CPDdemAut))
sumCPDdemHWWin <- (sum(CpdMergedHWDT$CPDdemWin))
sumCPDdemTotal <- sumCPDdemAut + sumCPDdemHWWin

sumPS1HW <- ((sumCPDdemHWWin / sumCPDdemTotal) + (sumCPDdemHWAut / sumCPDdemTotal)) * PS1 * 365
sumPS2HW <- ((sumCPDdemHWWin / sumCPDdemTotal) + (sumCPDdemHWAut / sumCPDdemTotal)) * PS2 * 365
sumPS3HW <- ((sumCPDdemHWWin / sumCPDdemTotal) + (sumCPDdemHWAut / sumCPDdemTotal)) * PS3 * 365
sumPS4HW <- ((sumCPDdemHWWin / sumCPDdemTotal) + (sumCPDdemHWAut / sumCPDdemTotal)) * PS4 * 365
