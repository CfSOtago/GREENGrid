...
#Input variables
EecaLight <- (1577 * 1000 * 1000 * 1000) / (1004329 + 453135) #Yearly Wh per household for lighting
Consumption2015 <- 
Consumption2017 <-
Consumption2019 <-
Consumption2021 <-
...
LightHHDT <- LightHHDT[, oriWh := meanW / 2, (meanW = NULL)]
LightHHDT <- LightHHDT[, meanW := NULL]
Scaling <- EecaLight / sum(LightHHDT$oriWh * 90)
LightHHDT <- LightHHDT[, oriWh := oriWh * Scaling] #Scaling profile to EECA input Wh
...
LightDT <- LightDT[, obsHalfHour := hms::trunc_hms(obsHourMin, 1800)]




LightHHDT <- LightDT[, .(oriWh = sum(oriWh),
                         oriMWhTot = sum(oriMWhTot),
                         LedWh = sum(LedWh),
                         LedMWhTot = sum(LedMWhTot)), keyby = .(obsHalfHour, season)]
...
#Visualising shifted and original power two colours
