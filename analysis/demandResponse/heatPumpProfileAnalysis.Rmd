...
#Calculating the amount added because of load shifting
sc3data <- sc3data[, DiffOrigMWh := ifelse(GWhs3 > 0, (GWhs3-GWhs1) * 1000, 0)]
...
Mergedsc3DT <- Mergedsc3DT[, neEcoValueHH := 0]
Mergedsc3DT <- Mergedsc3DT[, neEcoValueHH := ifelse(GWhs3 > 0, DiffOrigMWh * meanprice, 0)]
Mergedsc3DT <- Mergedsc3DT[, ecoValueHH := 0]
Mergedsc3DT <- Mergedsc3DT[, ecoValueHH := ifelse(DiffOrigMWh == 0,
                                                (GWhs1 * 1000) * meanprice, 0)]

neEcoValSumDT <- Mergedsc3DT[, .(sum(neEcoValueHH)),
                           keyby = .(Region, season, Period)]
setnames(neEcoValSumDT, old=c("V1"), new=c("neEcoVal"))


EcoValSumDT <- Mergedsc3DT[, .(sum(ecoValueHH)), keyby = .(Region, season, Period )]
setnames(EcoValSumDT, old=c("V1"), new=c("EcoVal"))

setkey(neEcoValSumDT, Region, season, Period)

setkey(EcoValSumDT, Region, season, Period)

MergedEcoValue <- neEcoValSumDT[EcoValSumDT]
