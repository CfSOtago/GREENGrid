...
MicroSimWinterHWDT <- MicroSimWinterHWDT[, powerLs := powerW]

TestDT <- copy(MicroSimWinterHWDT)
TestDT <- TestDT[, r_dateTime := NULL]
TestDT <- TestDT[, Period := NULL]
#TestDT <- TestDT[, powerm1:= powerW]
#TestDT <- TestDT[, powerm1:= ifelse(LsPeriod=="Off Peak 2"|"Off Peak 1 (Morning)"|"Off Peak 1 (Evening)"|"Evening Peak 2"|"Evening Peak 1"|"Morning Peak 2", NA, powerm1)]
TestDT <- TestDT[, power1 := powerW]
...
x <- chron(times=time)
data = TestDT %>% 
  select(-powerLsShifted, -powerWShifted, -powerNew) %>% 
  mutate(obsHourTime = obsHourMin +1-1) %>%
  data.table

data
unique(data$LsPeriod)
temp = data %>% 
  filter(LsPeriod == "Morning Peak 1") %>% 
  mutate(obsHourTime = obsHourTime -3*60*60)
temp %>% head
combineddata = data %>% 
...
combineddata 
View(combineddata %>% filter(powerW!=powernew))

# add powernew to testDT
appenedTables = rbind(combinedata1, combineddata2)

TestDTnew = TestDT %>% merge(combineddata %>% 
                               select(hhID, obsHourMin, dateNZ, powernew),
                             by = c('hhID','obsHourMin', 'dateNZ'), all.x = TRUE) %>% data.table()
TestDTnew %>% filter(LsPeriod=="Off Peak 1 (MP1 shifted)")
#__________________________________________________________
...
legend_title <- "Period"
myPlot <- ggplot2::ggplot(subset(MicroSimWinterHWDT, hhID %in% c("rf_45"))
