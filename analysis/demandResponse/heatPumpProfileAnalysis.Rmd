...
HotwaterObsDT <- HotwaterObsDT[, year := lubridate::year(r_dateTime)]
HotwaterObsDT <- HotwaterObsDT[, obsHourMin := hms::as.hms(r_dateTime)]
HotwaterObsDT <- HotwaterObsDT[, date := lubridate::date(r_dateTime,tz = "Pacific/Auckland")]
HotwaterObsDT <- HotwaterObsDT[, month := lubridate::month(r_dateTime)]
...
MicroSimWinterHWDT <- subset(MicroSimWinterHWDT, `r_dateTime` >= "2015-07-27 00:00:00" & `r_dateTime` <= "2015-08-02 23:59:00 ")#Make sure there are no public holidays or school holidays
...
legend_title <- "Year"
myPlot <- ggplot2::ggplot(subset(MicroSimWinterHWDT, hhID %in% c("rf_06", "rf_15"))
                          , aes(x = obsHourMin)) +
  geom_line(aes(y=powerW, colour = "rf_06"), size=0.2) +
