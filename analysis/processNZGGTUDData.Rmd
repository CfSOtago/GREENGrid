...
```{r clean unison data}
# Remove identifying data ----
tudUnisonAllDT <- tudUnisonAllDT[, c("Name","EmailAddress","IPAddress") := NULL]
```

```{r check Unison dates}
# Check for non-parsed diary dates
t <- head(tudUnisonAllDT[is.na(r_diaryDate),.(ResponseID,r_diaryDate, tudCode, StartDate, EndDate)])
...
The tudCodes are _not_ the gridSpy ids, we need to create these from the unison sheet in `r linkLUT`.
