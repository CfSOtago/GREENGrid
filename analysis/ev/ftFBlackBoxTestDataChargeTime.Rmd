...
Figure \@ref(fig:plotChargingTime) seems to show that the very high Amp values occured on one Sunday and we also appear to see some night charging 'not at home' which may indicate our location inference is imperfect. We re-draw this plot excluding the potential Amp errors (Figure \@ref(fig:plotChargingTimeClean)).
...
myPlot <- ggplot(plotDT[ampOutlier == "OK Amp data"], aes(x = rTime, y = meanPowerW/1000, colour = evID)) +
  geom_point() +
  facet_grid(evID + rDow ~ geoLoc) +
  labs(x='Time of Day', y = "Mean kW", caption = figCaption) +
  theme(legend.position = "bottom") #  legend
