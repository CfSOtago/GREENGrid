...
This work is (c) `r as.POSIXlt(Sys.time())$year + 1900` the University of Southampton. Re-use is governed by [this license](https://git.soton.ac.uk/ba1e12/nzGREENGrid/blob/master/LICENSE).
...
This document was created using [knitr](https://cran.r-project.org/package=knitr) in [RStudio](http://www.rstudio.com) with `r R.version.string` running on `r R.version$platform`. Some of the R code has been included where used for information and reference purposes. Full code is [available](https://git.soton.ac.uk/ba1e12/nzGREENGrid/tree/master/analysis/ev) as noted above.
...
That loaded `r nzGREENGrid::tidyNum(nrow(ftfDT))` observations.

## Create derived variables

Next we create some useful derived variables. Errors may be 'odd' dates and times or even not set inthe original data as we see below.

```{r process ftf data, echo = TRUE}
# create derived variables
ftfDT <- nzGREENGrid::createDerivedFtF(ftfDT)

t <- ftfDT[is.na(rDate), .(nObs = .N,
                           meanPackVolts = mean(`Pack volts`)), 
           keyby = .(`Date (GPS)`, `Time (GPS)`)]

knitr::kable(caption = "Number of obs and mean pack volts where date cannot be set by original GPS Date & Time", t)

t <- ftfDT[is.na(rTime), .(nObs = .N,
                           meanPackVolts = mean(`Pack volts`)), 
           keyby = .(`Date (GPS)`, `Time (GPS)`)]

knitr::kable(caption = "Number of obs and mean pack volts where time cannot be set by original GPS Date & Time", t)

```

We remove these NAs from the data (should we - what does GPS NA mean? no signal?).
...
Figure \@ref(fig:mapAllLocations) shows the location of all observations in the dataset (without sampling) coloured by `Speed (GPS)` If we selected just one vehicle and zoomed our map to the location at 01:00 - 04:00 when speed is 0 then we would probably determine their home. We could also determine other places visited at other times... This indicates how [disclosive GPS Lat/Long can be](http://toddwschneider.com/posts/analyzing-1-1-billion-nyc-taxi-and-uber-trips-with-a-vengeance/) even if address data is not provided.

```{r mapAllLocations, fig.cap="Map of all observations coloured by speed"}
# draw map of all GPS locations

evMap <- nzGREENGrid::getEVMap(ftfDT)
...
ftfDT <- nzGREENGrid::inferLocationFtF(ftfDT)
...
ftfSafeDT <- nzGREENGrid::createSafeFtF(ftfDT)
```
...
Questions to be aszked

 * Data:
    + Do the Amp & Volt distributions look right?
    + Cause of Amp outliers?
    + Date/Time NA (`r pcGPSNA` % of observations) are due to a lack of GPS date/time (and lat/long) - this means that there is no date/time in the data when GPS has no signal (?). Do we need another source of date/time?
 * Research:
