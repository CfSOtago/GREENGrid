...
dIPath <- paste0(ggParams$dataLoc, "raw/")
dOPath <- paste0(ggParams$dataLoc, "safe/")
```
 ...
 * test dataset stored at `r dIPath`
...
```{r get fileNames}
fileList <- list.files(path = dIPath , pattern = "EVBlackBox*")
fListDT <- as.data.table(fileList)
fListDT <- fListDT[, fullPath := path.expand(paste0(dIPath, fileList))]

kableExtra::kable(fListDT, caption = "Data files to be loaded") %>%
  kable_styling()
```

In this section we load, merge and describe the data files from `r ggParams$dataLoc`. 
...
geoDT <- head(ftfDT[, .(Longitude, Latitude, Altitude, ambient_temp_1)], 100)
...
# draw map of some EV locations

geoDT <- geoDT[Latitude != 0 & Longitude != 0]

# https://rstudio.github.io/leaflet/markers.html

leaflet::leaflet(data = geoDT[]) %>% addTiles() %>%
...
```{r checkFails}
message("Date (GPS)")
summary(ftfT$`Date (GPS)`)
message("rDate")
summary(ftfT$rDate)
message("Time (GPS)")
summary(ftfT$`Time (GPS)`)
message("rTime")
summary(ftfT$rTime)

message("rDateTime")
summary(ftfT$rDateTime)
...
```{r checkDates}
dateFail <- ftfT[is.na(rDate), .(`Date (GPS)`, rDate, `Time (GPS)`, rTime)]
...
```{r saveData}
of <- path.expand(paste0(dOPath, "ftfSafeLatestAll.csv"))
