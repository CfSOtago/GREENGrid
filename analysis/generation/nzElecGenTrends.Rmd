...
if(local){ # set data storage location
  ...
  ggParams$lDataLoc <- path.expand("/Volumes/hum-csafe/Research Projects/GREEN Grid/_RAW DATA/EA_Generation_Data/")
}

ggParams$figCaption <- paste0("Source: EA wholesale generation data 1997-2018",
                              "\nAvailable from: https://www.emi.ea.govt.nz/Wholesale/Datasets/Generation/Generation_MD/")
...
```{r boxPlot, fig.height=6, fig.cap = "Box plot of energy produced by all fuels"}
...
```{r overallHistograms, fig.height=7, fig.cap = "Overall energy production histrograms by fuel type"}
...
makeMonthlyPlot <- function(dt, yVar, yLabel){
...
makeMonthlyPlot(plotDT, "sumkWh", "Total GWh")
```
...
makeMonthlyPlot(plotDT, "meankWh", "Mean GWh")
```
...
makeMonthlyPlot(plotDT, "sdkWh", "S.D. GWh")
```
...
makeHalfHourlyColPlot <- function(dt, yVar, yLabel){
  #print(paste0("Making monthly plot for ", eval(yVar), " with y label ", eval(yLabel)))
  ggplot(plotDT, aes(x = rTime, y = get(yVar)/1000000, fill = Fuel_Code)) +
    geom_col(alpha = 0.4) +
    facet_grid(month ~ year) +
    labs(caption = ggParams$figCaption, y = eval(yLabel), x = "Time of Day") +
...
makeHalfHourlyPointPlot <- function(dt, yVar, yLabel){
  #print(paste0("Making monthly plot for ", eval(yVar), " with y label ", eval(yLabel)))
  ggplot(plotDT, aes(x = rTime, y = get(yVar)/1000000, colour = Fuel_Code)) +
...
makeHalfHourlyColPlot(plotDT, "sumkWh", "Sum GWh")
```

```{r sumProfilePoint, fig.height=6, fig.cap="Half-hourly profile plot by fuel type (sum)"}
makeHalfHourlyPointPlot(plotDT, "sumkWh", "Sum GWh")
```
...
makeHalfHourlyColPlot(plotDT, "meankWh", "Mean GWh")
```

```{r meanProfilePoint,fig.height=6,fig.cap= "Half-hourly profile plot by fuel type (mean)"}
makeHalfHourlyPointPlot(plotDT, "meankWh", "Mean GWh")
```
...
makePointProfilePlot <- function(dt, title){
  ...
  ggplot(dt, aes(x = rDateTime, y = kWh/1000, colour = Fuel_Code)) +
    geom_point(alpha=0.4) +
...
makePointProfilePlot(sampleGenDT[year == 1998 & month == 6], "June 1998")
makePointProfilePlot(sampleGenDT[year == 1998 & month == 12], "December 1998")
makePointProfilePlot(sampleGenDT[year == 2017 & month == 6], "June 2017")
makePointProfilePlot(sampleGenDT[year == 2017 & month == 12], "December 2017")
```
...
makeColProfilePlot(sampleGenDT[year == 1998 & month == 6], "June 1998")
makeColProfilePlot(sampleGenDT[year == 1998 & month == 12], "December 1998")
makeColProfilePlot(sampleGenDT[year == 2017 & month == 6], "June 2017")
makeColProfilePlot(sampleGenDT[year == 2017 & month == 12], "December 2017")
```
...
makePlotPoint(plotDT, xVar = "year", yVar = "sumkWh", xLabel = "Year", yLabel = "Total GWh")
```
...
```{r dailyTotalPlotPoint, fig.cap="Yearly total plot by fuel type"}
plotDT <- allGenDT[, 
                .(sumkWh = sum(kWh, na.rm = TRUE),
                  meankWh = mean(kWh, na.rm = TRUE),
                  mediankWh = median(kWh, na.rm = TRUE),
...
yMax <- max(plotDT$sumkWh)
myAlpha <- 0.1
vLineAlpha <- 0.4
vLineCol <- "#0072B2" # http://www.cookbook-r.com/Graphs/Colors_(ggplot2)/#a-colorblind-friendly-palette
myTextSize <- 3

annotateAllGen <- function(plot){
  plot <- plot +
    annotate("text", x = "2004-01-01",
             y = yMax * 0.9, angle = 10,size = myTextSize,
             label = "Huntly 50 MW gas unit added", hjust = 0) +
    geom_vline(xintercept = "2004-01-01", alpha = vLineAlpha, colour = vLineCol) +
    annotate("text", x = "2007-01-01",
             y = yMax * 0.9, angle = 10,size = myTextSize,
             label = "Huntly CCGT unit added", hjust = 0) +
    geom_vline(xintercept = "2007-01-01", alpha = vLineAlpha, colour = vLineCol) +
    annotate("text", x = "2007-06-01",
             y = yMax * 0.9, angle = 10,size = myTextSize,
             label = "Huntly mainly gas fired", hjust = 0) +
    geom_vline(xintercept = "2007-06-01", alpha = vLineAlpha, colour = vLineCol) +
    annotate("text", x = "2008-06-01",
             y = yMax * 0.9, angle = 10,size = myTextSize,
             label = "Dry winter", hjust = 0) +
    geom_vline(xintercept = "2008-06-01", alpha = vLineAlpha, colour = vLineCol) +
    annotate("text", x = "2012-12-01",
             y = yMax * 0.9, angle = 10,size = myTextSize,
             label = "1 of 4 Huntly Power Station 250 MW units mothballed", hjust = 0) +
    geom_vline(xintercept = "2012-12-01", alpha = vLineAlpha, colour = vLineCol) +
    annotate("text", x = "2013-06-01",
             y = yMax * 0.9, angle = 10,size = myTextSize,
             label = "2 of 4 Huntly Power Station 250 MW units in storage", hjust = 0) +
    geom_vline(xintercept = "2013-06-01", alpha = vLineAlpha, colour = vLineCol) +
    annotate("text", x = "2015-06-01",
             y = yMax * 0.9, angle = 10,size = myTextSize,
             label = "2 of 4 Huntly Power Station 250 MW units retired", hjust = 0) +
    geom_vline(xintercept = "2015-06-01", alpha = vLineAlpha, colour = vLineCol)
  return(plot)
}

# final plot - adds annotations
annotateAllGen(myPlot)
