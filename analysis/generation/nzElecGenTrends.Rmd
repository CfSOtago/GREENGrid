...
Table \@ref(tab:finalSummary) sumamrises the data. Notice that there are missing (NA) values in the kWh column. These are caused by periods 49 and 50:
...
makeHalfHourlyColPlot(plotDT, "sumkWh", "Sum GWh per half hour")
```

```{r sumProfilePoint, fig.height=6, fig.cap="Half-hourly profile plot by fuel type (sum)"}
makeHalfHourlyPointPlot(plotDT, "sumkWh", "Sum GWh per half hour")
```
...
makeHalfHourlyPointPlot(plotDT, "meankWh", "Mean GWh per half hour")
```
...
myPlot <- makePlotCol(plotDT, xVar = "year", yVar = "sumkWh", xLabel = "Year", yLabel = "Total yearly GWh")
...
myPlot <- makePlotPoint(plotDT, xVar = "year", yVar = "sumkWh", xLabel = "Year", yLabel = "Total yearly GWh")
annotateAllGenYears(myPlot)
```

## Trends by day

The following plots show the same trends but using daily data to highlight seasonal patterns over time. Annotations use information from:
...
# point plot
myPlot <- makePlotPoint(plotDT, xVar = "rDate", yVar = "sumkWh", xLabel = "Year", yLabel = "Total daily GWh")
...
# col plot
myPlot <- makePlotCol(plotDT, xVar = "rDate", yVar = "sumkWh", xLabel = "Year", yLabel = "Total daily GWh")
myPlot +
  scale_x_date(date_labels = "%b %Y", date_breaks = "6 months", limits = dateLims) +
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust = 0.5)) +
  labs(title = myTitle)
