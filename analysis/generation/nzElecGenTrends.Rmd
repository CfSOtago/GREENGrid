...
naPlotDT <- genDT[, .(nObs = .N), keyby = .(rYear, rMonth, Time_Period, Fuel_Code, kWhisNA)] # table them for plot

# plot the NAs by Time_Period to look for patterns
ggplot(naPlotDT[kWhisNA == "NA"], aes(x = rMonth, y = Time_Period, fill = nObs)) + 
  geom_tile() +
