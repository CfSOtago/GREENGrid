...
pattern <- "Heat Pump"
gs1MinDT <- getCleanGridSpyData(fListDT[, fullPath]) # beware, loads all data
...
```{r select best data and plot mean Heat Pump power}
# add season
...
myPlot <- ggplot(plotDT[metaDT], aes(y = meanW, x = obsHourMin, colour = hhID)) +
  geom_point()

myPlot + facet_grid(season ~ nAdults) +
