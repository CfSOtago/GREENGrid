...
So we are looking for `r circuitPattern` circuits between `r dateFrom` and `r dateTo`.
...
Loaded data observation plots - just to confirm what `r circuitPattern` data we have. 
...
myPlot <- ggplot2::ggplot(plotDT[metaDT], aes( x = as.Date(date), y = hhID, 
  ...
  theme(axis.text.x = element_text(angle = 90, 
                                   vjust = 0.5,
                                   hjust = 0.5)) + 
  labs(title = paste0("N observations per household per day for: ", circuitPattern),
       caption = myCaption
  )
...
myPlot <- ggplot2::ggplot(plotDT, aes( x = as.Date(date), y = nHH, fill = sample)) +
  ...
  theme(axis.text.x = element_text(angle = 90, 
                                   vjust = 0.5, 
                                   hjust = 0.5)) + 
  labs(title = paste0("N households per day for: ", circuitPattern),
       caption = myCaption
  )
myPlot
...
```{r histo}
myCaption <- paste0("Data source: ", outPath,
                    "\nUsing data from ",  min(gs1MinDT$r_dateTime), " to ", max(gs1MinDT$r_dateTime))

ggplot2::ggplot(gs1MinDT, aes(x = powerW)) +
  geom_histogram() +
  labs(title = paste0("Distribution of ", circuitPattern, " power values"),
       caption = myCaption)
```
...
myPlot <- ggplot2::ggplot(plotDT, aes(y = meanW, x = obsHourMin, colour = hhID)) +
  geom_point() +
  facet_grid(season ~ .) +
  labs(title = paste0("Mean power profiles for: ", circuitPattern, " circuits"),
       caption = myCaption)
myPlot
