...
processUnison <- function(dt){
  ...
  # Fix hhID ----
  dt <- dt[, hhID := paste0("check_rf_", substr(code, 0, 2))] # extracts char 1
}
...
t <- head(tudUnisonAllDT[is.na(r_diaryDate),.(r_diaryDate, hhID)])
...
ggplot2::ggplot(plotDT, aes(x = as.Date(r_diaryDate), y = nDiaries)) +
  geom_point() +
    scale_x_date(date_labels = "%a %d %b %Y", date_breaks = "1 month") +
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust = 0.5)) + 
