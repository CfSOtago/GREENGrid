...
## Testing specific months using plotly

```{r checkMonth}
testDT <- gsDT[month == "Oct" & year == "2016"]
plotDT <- testDT[, .(meanW = mean(powerW)), keyby = .(obsDate, obsTime, circuit)]

myPlot <- ggplot2::ggplot(plotDT, aes(x = obsTime, y = meanW, colour = circuit)) + 
  geom_line() +
  facet_wrap(. ~ obsDate)
```
