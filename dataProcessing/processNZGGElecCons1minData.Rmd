...
The data has been saved in 'long' form so that each file only has 4 columns:

 * hhID: household id
 * r_dateTime: time of observation
 * circuit: the circuit label
 * powerW: power observation
 
As an example, here are the first few rows of one of the files:

```{r data header}
t <- head(tempHhLongDT)
kable(caption = "First 6 rows of example data for 1 household", t)
```
